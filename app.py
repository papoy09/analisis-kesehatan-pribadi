    ...
    # ── STEP 2 ──────────────────────────────────────────────────────────
        ...
        if not st.session_state.basic_data:
            ...
            with st.form("form_step2"):
                ...
                with a1: sys_bp = st.number_input("Systolic BP",        0, 300, 120)
                with b1: dia_bp = st.number_input("Diastolic BP",       0, 200, 80)
                with c1: rhr   = st.number_input("Resting Heart Rate", 0, 250, 75)
                with d1: hrv   = st.number_input("HRV",                0, 200, 50)

                st.markdown("<hr>", unsafe_allow_html=True)
                st.markdown("**🧪 Profil Lipid**")
                a2, b2, c2, d2 = st.columns(4)
                with a2: chol = st.number_input("Cholesterol",   0.0, 600.0,  200.0, format="%.1f")
                with b2: ldl  = st.number_input("LDL",           0.0, 400.0,  100.0, format="%.1f")
                with c2: hdl  = st.number_input("HDL",           0.0, 200.0,   50.0, format="%.1f")
                with d2: trig = st.number_input("Triglycerides", 0.0, 1000.0, 150.0, format="%.1f")

                st.markdown("<hr>", unsafe_allow_html=True)
                st.markdown("**🩸 Metabolik & Gula Darah**")
                a3, b3, c3, d3 = st.columns(4)
                with a3: gluc  = st.number_input("Glucose Level", 0.0, 600.0, 90.0, format="%.1f")
                with b3: hba1c = st.number_input("HbA1c (%)",     0.0,  20.0,  5.0, format="%.1f")
                with c3: crp   = st.number_input("CRP",           0.0, 100.0,  1.0, format="%.2f")
                with d3: egfr  = st.number_input("eGFR",          0.0, 200.0, 90.0, format="%.1f")

                st.markdown("<hr>", unsafe_allow_html=True)
                st.markdown("**🧬 Data Genomik**")
                a4, b4, c4, d4 = st.columns(4)
                with a4: prs_c = st.number_input("PRS Cardiometabolic", value=0.0, format="%.4f")
                with b4: prs_d = st.number_input("PRS Type2Diabetes",   value=0.0, format="%.4f")
                with c4: apoe  = st.selectbox("APOE e4 Carrier",        [0, 1], format_func=lambda x: "Tidak" if x == 0 else "Ya")
                with d4: brca  = st.selectbox("BRCA Pathogenic Variant",[0, 1], format_func=lambda x: "Tidak" if x == 0 else "Ya")

                st.markdown("<br>", unsafe_allow_html=True)
                bc1, bc2, bc3 = st.columns([4, 2, 2])
                with bc1: submitted2 = st.form_submit_button("🔍 Prediksi Sekarang", use_container_width=True)
                with bc3: back_btn  = st.form_submit_button("← Kembali ke Step 1",  use_container_width=True)
        ...
        try:
            ...
            if missing:
                ...
                if st.button("🚀 Jalankan Prediksi"):
                    ...
                    st.markdown("<hr>", unsafe_allow_html=True)
                    st.download_button("⬇ Download Hasil CSV", result_bytes, "hasil_prediksi.csv", "text/csv")
